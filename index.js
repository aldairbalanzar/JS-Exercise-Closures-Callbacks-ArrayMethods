...
function processSum(list, cb) {
  return cb(list.reduce((acc, currVal) => acc + currVal, 0));
}
...
function processDuplicateFree(list, cb) {
  var newList = list.filter((item, index) => list.indexOf(item) === index)
  return cb(newList);
}
...
function tallyUpDonations(runners) {
  return runners.reduce((acc, currVal) => acc + currVal.donation, 0);
}
